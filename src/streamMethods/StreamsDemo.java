...
class Employee
{
    ...
    public Employee(Integer id, String name, Integer yearsInOrg, double salary) {
		super();
		this.id = id;
		this.name = name;
		this.yearsInOrg = yearsInOrg;
	...
	public static void main(String[] args) {
		 List<Employee> lstEmp = Employee.getEmployeeList();
	        ...
	        List<Employee> lstNewEmp = lstEmp.stream().filter(emp -> emp.getYearsInOrg() < 1)
	                .map(e -> { e.setId(e.getId() + 5000); return e; })
	                .collect(Collectors.toList());
	        lstNewEmp.forEach((e) -> System.out.println(e.getId() + ":" + e.getName() + ":" + e.getId())); 
	}
